...
Knowledge Agent supports parallel execution of A2A calls when the LLM invokes multiple sub-agents simultaneously. This can significantly reduce response times when queries require information from multiple agents.
...
a2a:
  ...
  # Enable parallel execution of A2A calls (default: false)
  parallel_execution: true

  # Maximum concurrent A2A calls (default: 5)
...
1. When the LLM requests multiple sub-agents simultaneously, calls are batched
2. The ParallelExecutor runs all calls concurrently (respecting `max_concurrent_calls`)
3. Results are collected and returned, even if some calls fail (partial results)
4. The total latency is approximately the slowest individual call, not the sum

### Example Performance

**Sequential execution (parallel_execution: false):**
```
metrics_agent: 2.5s
logs_agent: 1.8s
alerts_agent: 3.2s
...
```
metrics_agent: 2.5s  ─┐
logs_agent: 1.8s     ─┼─ All run concurrently
alerts_agent: 3.2s   ─┘
─────────────────────
Total: ~3.2s (max of individual times)
```

### Partial Failure Handling

When parallel execution is enabled, individual failures don't block other results:

- Successful responses are included in the result
- Failed calls include the error in the response
- The LLM receives all available information and can handle partial data

### Metrics

Parallel execution is tracked with dedicated metrics:

| Metric | Description |
|--------|-------------|
| `knowledge_agent_a2a_parallel_batches_total` | Total parallel batch executions |
| `knowledge_agent_a2a_parallel_batch_size` | Number of calls per batch |
| `knowledge_agent_a2a_parallel_batch_latency_seconds` | Total batch execution time |
| `knowledge_agent_a2a_parallel_batch_success_rate` | Success rate (0.0-1.0) per batch |

### Limitations

- Only applies when the LLM requests multiple sub-agents in the same turn
- Individual A2A calls still execute sequentially within ADK's base flow
- Context is shared across parallel calls (same user identity, session, etc.)
