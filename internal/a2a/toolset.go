...
func NewA2AToolset(ctx context.Context, cfg *config.A2AConfig, keycloakClient *keycloak.Client) (*A2AToolset, error) {
	...
	for _, subAgentCfg := range cfg.SubAgents {
		t, client, err := createSubAgentTool(ctx, subAgentCfg, cfg, keycloakClient)
		if err != nil {
...
// createSubAgentTool creates a tool for a single sub-agent
func createSubAgentTool(ctx context.Context, cfg config.A2ASubAgentConfig, a2aCfg *config.A2AConfig, keycloakClient *keycloak.Client) (tool.Tool, *a2aclient.Client, error) {
	log := logger.Get()
...
// Close cleans up all A2A clients
func (ts *A2AToolset) Close() error {
	...
	if len(errs) > 0 {
		...
		return fmt.Errorf("errors closing A2A clients: %v", errs)
	}
